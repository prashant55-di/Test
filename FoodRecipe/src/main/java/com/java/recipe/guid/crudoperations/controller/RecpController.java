...
public class RecpController {
	...
	public Object getRecipeEntityById(@PathVariable(value="id")int recipeid) throws RecipeNotFoundException 
	{
	...
	public ResponseEntity<Void> deleteRecipe(@PathVariable(value="id")int recipeid) throws RecipeNotFoundException{

		return recipeService.deleteById(recipeid);
}

	@PutMapping("/recipees/{id}")
	public RecipeEntity updateRecipe(@Validated @RequestBody RecipeEntity recipeEnity)throws RecipeNotFoundException
	{
		return recipeService.updateRecipe(recipeEnity);
		//return recipeService.updateRecipe(recipeid, recipeEnityDetails); 
		//recipeEnityDetails).ok(HttpStatus.OK);
